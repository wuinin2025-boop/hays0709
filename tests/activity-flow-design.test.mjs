...
assert.match(html, /\.poster-card \{\n[\s\S]*?min-height: 960px;[\s\S]*?grid-template-rows: auto auto;[\s\S]*?border: 1px solid rgba\(200, 251, 232, \.42\);[\s\S]*?border-radius: 6px;/);
assert.match(html, /\.poster-qr-block \{\n[\s\S]*?margin-top: 30px;/);
assert.match(html, /<div class="qr-crop">\s*<img class="qr-wire" src="assets\/hays-qr\.png" alt="Hays 官网二维码">\s*<\/div>/);
